...
#507 offers three ways out. They are quoted here so this section stands on its own:

> 1. **Clear and re-set:** read the value, append the typed text, write it back through the native setter (what `fill` does), keeping "type appends" semantics for number inputs.
> 2. **Refuse:** error on `type`/`press` against `input[type=number]` and point the caller at `fill`, on the grounds that key-by-key typing into a number input is not meaningful.
> 3. **Document the limitation and leave behavior as is.**
...
python3 verify-blocks.py ../README.md <stock-dir>   # re-runs the report's own blocks
```
